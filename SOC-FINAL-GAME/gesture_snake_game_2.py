...
# ---------------------------------------------------------
# TRAIL POINTS (DEQUE)
# We use a deque (double-ended queue) with a maximum length of 20.
# This automatically removes the oldest point when a new one is added,
# creating a smooth, fading visual trail for the dynamic swipe gesture.
# ---------------------------------------------------------
trail_points = deque(maxlen=20)

# --- MediaPipe Setup ---
mp_hands         =  mp.solutions.hands       # Load the MediaPipe Hands model
mp_drawing_tools =  mp.solutions.drawing_utils # Utility for drawing hand landmarks

# Initialize the Hands object with specific confidence thresholds
...
# =====================================================================
# 2. HELPER FUNCTIONS & GESTURE RECOGNITION
# =====================================================================

# --- Extract Landmark Coordinates ---
def get_cordinates(img, hand_landmark):
    lm_list = []
    h, w = img.shape[:2] # Get image dimensions for scaling
    
    # Loop through all 21 landmarks and convert normalized values to pixel coordinates
...
# Calculates the distance between two landmarks (p1 and p2).
# To make it scale-invariant (works whether the hand is close to
# or far from the camera), we divide the raw distance by a reference 
# distance (from the wrist to the middle finger MCP joint).
# ---------------------------------------------------------
def normalised_distance(lm_list, p1, p2):
    x1, y1 = lm_list[p1][1], lm_list[p1][2]
    x2, y2 = lm_list[p2][1], lm_list[p2][2]
    
    distance = math.hypot(x2 - x1, y2 - y1) # Raw Euclidean distance
    
    # Reference distance from wrist (0) to middle finger base (9)
    ref = math.hypot(lm_list[0][1] - lm_list[9][1], lm_list[0][2] - lm_list[9][2])
    
    if ref == 0:
        return distance # Prevent division by zero
    else:
        return distance / ref


# ---------------------------------------------------------
# STATIC GESTURE CREATOR
# Determines the current static hand posture based on which fingers
# are open or closed. It checks the y-coordinates of finger tips
# against their lower joints. It also uses the normalized distance
# between the thumb and index finger to detect a "PINCH" gesture.
# ---------------------------------------------------------
def finger_oreo_hand(lm_list, hand_label):
    tips = [8, 12, 16, 20] # Tip landmark IDs for Index, Middle, Ring, Pinky

    # Calculate normalized distance between thumb tip (4) and index tip (8)
    distance = normalised_distance(lm_list, 4, 8)

    Orientation = []
    # Check thumb orientation depending on left or right hand
    if hand_label == "Right":
        Orientation.append(1 if lm_list[3][1] > lm_list[4][1] else 0)
    else :
        Orientation.append(1 if lm_list[3][1] < lm_list[4][1] else 0)
        
    # Check if the other 4 fingers are extended (tip above the PIP joint)
    for tip in tips :
        Orientation.append(1 if lm_list[tip][2] < lm_list[tip-2][2] else 0)
        
    gesture = "NONE"
    # Map the resulting array of 0s and 1s to specific gestures
...
# Variables for tracking dynamic swipes
current_point = (0,0)
...
# ---------------------------------------------------------
# DYNAMIC GESTURE (SWIPE DETECTION)
# Tracks the index finger's tip over consecutive frames to calculate
# the change in X (dx) and Y (dy). By keeping a count of continuous
# directional movements (and ignoring slight diagonal jitters),
# it accurately identifies UP, DOWN, LEFT, or RIGHT swipes.
# ---------------------------------------------------------
def dynamic_gesture(lm_list):
    ...
    current_point = lm_list[8][1:] # Get (x,y) of index finger tip
    
    dx = current_point[0] - previous_point[0] # Calculate horizontal shift
    dy = current_point[1] - previous_point[1] # Calculate vertical shift
    
    previous_point   = current_point
    trail_points.appendleft(current_point) # Add point to trail deque
    ...
    # Finalize swipe gesture if count threshold is met
...
def random_food(snake_pos):
    while True:
        ...
        if pos not in snake_pos: # Ensure food doesn't spawn on the snake
            break
    return pos
...
def draw_cell(surface, color, pos):
    X = CELL_SIZE * pos[0]
    Y = CELL_SIZE * pos[1]
    pygame.draw.rect(surface, color, (X, Y, CELL_SIZE, CELL_SIZE)) # Fill cell
    pygame.draw.rect(surface, BG_COLOR, (X, Y, CELL_SIZE, CELL_SIZE), 1) # Draw cell border
...
def main():
    ...
    pygame.display.set_caption('GESTURE CONTROLLED ARCADE GAME - SNAKE')
    ...
    # Helper function to restore default game state
    def reset_game():
    ...
    # Gesture tracking dictionaries for Left and Right hands
    ...
    while True:
        # --- PHASE 1: CAPTURE & PROCESS CAMERA FEED ---
        success, frame = cap.read()
        if not success:
            print("Camera not found")
            break
            
        img     = cv.flip(frame, 1) # Mirror image for intuitive control
        rgb_img = cv.cvtColor(img, cv.COLOR_BGR2RGB) # Convert to RGB for MediaPipe
        ...
        if results.multi_handedness and results.multi_hand_landmarks :
            for handedness , hand_landmark in zip(results.multi_handedness, results.multi_hand_landmarks):             
                ...
                # To prevent the game from glitching due to momentary 
                # misdetections, we require a gesture to be held for a 
                # certain number of consecutive frames (STABILITY_FRAMES) 
                # before it is officially registered and used by the game.
                # ---------------------------------------------------------
                ...
                # Check for swipe gestures specifically on the right hand
                if hand_label == "Right" :
                    swipe_gesture[hand_label] = dynamic_gesture(lm_list)
                
                # Draw the visual trail for the index finger    
            ...
            # Reset left hand gesture state if it disappears from frame
            if "Left" not in hands_present_this_frame:
                GESTURE['Left'] = 'NONE'
                PREVIOUS_GESTURE['Left'] = 'NONE'
                GESTURE_COUNT['Left'] = 0
                
            # Reset right hand gesture state if it disappears from frame
            ...
            # Clear all states if no hands are detected
            trail_points.clear()
            swipe_gesture = {'Left' :'NONE', 'Right' :'NONE'}
            GESTURE   = {'Left' :'NONE', 'Right' :'NONE'}
            
            
        # --- PHASE 2: PYGAME EVENT HANDLING & GAME LOGIC ---
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            if event.type == pygame.KEYDOWN :
                if event.key == pygame.K_q : # Quit game on 'Q' press
                    pygame.quit()
                    sys.exit()       
        ...
        # Active gameplay logic
            ...
            if not GAME_PAUSED :
                if GESTURE['Left'] == 'OPEN PALM' :
                    GAME_PAUSED = True 
                # Prevent 180-degree turns to avoid immediate self-collision
                ...
                if GESTURE['Left'] == "FIST": # Resume game from pause
                    GAME_PAUSED = False
                    
        current_time = pygame.time.get_ticks()
        
        # --- GAME UPDATING (Throttled by MOVE_DELAY) ---
        if GAME_STARTED and not GAME_OVER and not GAME_PAUSED and (current_time - LAST_MOVE_TIME > MOVE_DELAY):
            ...
            if HIT_SELF or HIT_WALL :
                ...
                snake_pos.insert(0, NEW_HEAD) # Move snake forward
                if NEW_HEAD == food_pos :
                    score += 1
                    food_pos = random_food(snake_pos)
                    MOVE_DELAY = max(MOVE_DELAY_MIN , MOVE_DELAY - SPEED_DELAY) # Increase speed upon eating
                else :
                    snake_pos.pop() # Remove tail segment if no food eaten
                    
                    
        # --- PHASE 3: RENDERING (PyGame) --- 
        ...
        if not GAME_STARTED :
            ...
            # ---------------------------------------------------------
            # GRID DRAWING
            # Draws horizontal and vertical lines across the Pygame screen
            # to create a classic arcade grid background, making it easier
            # for the player to judge distances and snake alignment.
            # ---------------------------------------------------------
            ...
            draw_cell(screen, FOOD_COLOR , food_pos) # Draw the food
            for i , part in enumerate(snake_pos):
                color = SNAKE_HEAD if i == 0 else SNAKE_BODY
                draw_cell(screen , color , part) # Draw snake segments
                
            # Render HUD (Score and High Score)
        ...
        # --- PHASE 4: RENDERING (OpenCV Debug View) ---
        ...
        if cv.waitKey(1) & 0xFF == ord('q'): # Allow exiting via OpenCV window
            break

...
def hand_oreo(lm_list , hand_label):
    ...
    index_tip = lm_list[8]
    index_mcp = lm_list[5]
    angle_radians = math.atan2( index_tip[2] - index_mcp[2], index_tip[1]-index_mcp[1])
...
def main():
    ...
    while True:
        ...
        if not GAME_STARTED :
            ...
            if not GAME_PAUSED :
                if STABLE_GESTURE['Right'] == 'OPEN PALM' :
                    GAME_PAUSED = True 
                elif STABLE_GESTURE['Right'] == 'POINT UP' and direction != (0, 1):
                    direction = (0, -1)
                elif STABLE_GESTURE['Right'] == 'POINT DOWN' and direction != (0, -1) :
                    direction = (0, 1)
                elif STABLE_GESTURE['Right'] == 'POINT LEFT' and direction != (1, 0) :
                    direction = (-1, 0)
                elif STABLE_GESTURE['Right'] == 'POINT RIGHT' and direction != (-1, 0) :
                    direction = (1, 0)
            else :
                if STABLE_GESTURE['Right'] in ["POINT UP", "POINT DOWN", "POINT LEFT", "POINT RIGHT"]:
                    GAME_PAUSED = False
